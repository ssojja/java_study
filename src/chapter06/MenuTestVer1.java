	...
	public static void main(String[] args) {
		...
		while(menuFlag) {
			...
			if(menu == 1) {
				...
				if(menuCount == MAX_SIZE) System.out.println("메뉴가 모두 등록되었습니다.");

				for(int i = menuCount; i < MAX_SIZE; i++) {
					System.out.print("메뉴명 > ");
					nameList[i] = scan.next();
					
					System.out.print("이모지명 > ");
				...
				if(menuCount != 0) {
					...
					System.out.println("=> 등록된 메뉴가 없습니다. 등록을 먼저 진행해주세요.");
				}
			} else if(menu == 3){
				if(menuCount != 0) {
					...
					while(searchFlag) {
						...
						System.out.print("계속 검색하시겠습니까? (계속:아무키나 누르세요, 종료:n) > ");
						if(scan.next().equals("n")) {
							searchFlag = false;
							System.out.println("=> 검색 종료되었습니다.");
						}
					} // while - searchFlag
				} else {
					System.out.println("=> 등록된 메뉴가 없습니다. 등록을 먼저 진행해주세요.");
